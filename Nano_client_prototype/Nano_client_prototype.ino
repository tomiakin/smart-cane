...
// Define pins for vibration

#define haptic_pin D3 // This pin sends out the vibration signal
...
void loop() {
  ...
  if (connected) {
    ...
    String distance = String(rxValue.c_str());
    Serial.println(distance);
    //double dist_vib = atof(distance.c_str());
    //Serial.println(dist_vib);

    // double vibration_power = 255*exp(-0.08 * dist_vib);

    // if (distance.length() > 0) {  // checks if variable is not empty
    //   digitalWrite(LED_BUILTIN, HIGH);
    //   analogWrite(haptic_pin, int(vibration_power));
    // } else {
